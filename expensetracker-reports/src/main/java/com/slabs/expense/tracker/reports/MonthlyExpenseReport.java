...
public class MonthlyExpenseReport extends ExpenseTrackerReport {
	...
	public MonthlyExpenseReport(UserInfo userInfo, List<Income> monthlyIncome, Month month, Integer year)
			throws InstantiationException, IllegalAccessException {
		this(userInfo, monthlyIncome, month, year, CurrencyType.USD);
	}

	public MonthlyExpenseReport(UserInfo userInfo, List<Income> monthlyIncome, Month month, Integer year, CurrencyType currency)
			throws InstantiationException, IllegalAccessException {
	...
	private void addReportDetails() {
		...
		TextFieldBuilder<String> title = componentBuilder.text("Report Owner").setStyle(styleProvider.getBoldStyle(12).setUnderline(Boolean.TRUE));
	...
	private void addIncomeDetails() {
		...
		TextFieldBuilder<String> title = componentBuilder.text("Income Details").setStyle(styleProvider.getBoldStyle(12).setUnderline(Boolean.TRUE));
		...
		TextFieldBuilder<String> totIncome = componentBuilder.text("Total Income : " + getCurrency().currencySymbol() + " " + totalIncome);
		detailContainer.add(totIncome).newRow();

		verticalContainer.add(title, componentBuilder.verticalGap(5), detailContainer, componentBuilder.verticalGap(10), expenseTitle,
				componentBuilder.verticalGap(5));
